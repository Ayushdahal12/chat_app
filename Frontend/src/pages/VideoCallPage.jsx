...
// UI Components (Icons) kept exactly as provided
...
const VideoCallPage = () => {
  ...
  const durationRef = useRef(0); // For logging accuracy
  ...
  // Update durationRef so the endCall function always has the latest value
  useEffect(() => {
  ...
  // ✅ LOG CALL TO BACKEND BEFORE EXITING
  ...
  const toggleVideo = () => {
    ...
    if (videoTrack) {
      ...
      // Notify remote peer by updating the stream via RTPSender if necessary
      // But standard 'enabled = false' sends black frames, which is correct for Messenger style
    }
  };

  // ✅ FIX: Mic Turn Off Logic
  const toggleAudio = () => {
